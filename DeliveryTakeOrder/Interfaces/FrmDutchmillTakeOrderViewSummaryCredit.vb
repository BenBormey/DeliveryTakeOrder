...
Public Class FrmDutchmillTakeOrderViewSummaryCredit
    ...
    Private Sub FrmDutchmillTakeOrderViewSummaryCredit_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadingInitialized()
    ...
    Private Sub BtnExportToExcel_Click(sender As Object, e As EventArgs) Handles BtnExportToExcel.Click
        If DgvShow.RowCount > 0 Then
            ...
            For Each QsDataRow As DataGridViewRow In DgvShow.Rows
                ...
                RSheet.Range("E" & QsRow).Value = IIf(IsDBNull(QsDataRow.Cells("GrandTotal").Value) = True, "", QsDataRow.Cells("GrandTotal").Value)
                RSheet.Range("F" & QsRow).Value = IIf(IsDBNull(QsDataRow.Cells("CreditLimit").Value) = True, "", QsDataRow.Cells("CreditLimit").Value)
                RSheet.Range("G" & QsRow).Value = IIf(IsDBNull(QsDataRow.Cells("CreditLimitAllow").Value) = True, "", QsDataRow.Cells("CreditLimitAllow").Value)
                RSheet.Range("H" & QsRow).Value = IIf(IsDBNull(QsDataRow.Cells("LatestInvoice").Value) = True, "", QsDataRow.Cells("LatestInvoice").Value)
                RSheet.Range("I" & QsRow).Value = IIf(IsDBNull(QsDataRow.Cells("OverCredit").Value) = True, "", QsDataRow.Cells("OverCredit").Value)
                QsRow = QsRow + 1
            Next
    ...
    Private Sub DgvShow_DoubleClick(sender As Object, e As EventArgs) Handles DgvShow.DoubleClick
        ...
        With DgvShow.Rows(DgvShow.CurrentRow.Index)
            ...
            Dim vFrm As New FrmDutchmillTakeOrderViewCredit With {.vPlanning = vPlanning, .vDepartment = vDepartment, .vCusNum = vCusNum}
